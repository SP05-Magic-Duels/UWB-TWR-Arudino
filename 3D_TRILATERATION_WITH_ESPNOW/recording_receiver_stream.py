...
def parse_position(line):
    if not line.startswith("From: "):
        return None
    try:
        mac_addr = line.split(" | ")[0].split(": ")[-1]
        parts = line.split(" | ")[-1].split(": ")[-1]
        parts = parts.split(', ')
        if len(parts) != 3:
            return None
        x, y, z = float(parts[0]), float(parts[1]), float(parts[2])
        return mac_addr, x, y, z
    except ValueError:
        return None
...
def main():
    ...
    with open(CSV_FILE, 'w', newline='') as csvfile:
        ...
        writer.writerow(['timestamp', 'mac_addr', 'x', 'y', 'z'])  # Header
            ...
            while True:
                if ser.in_waiting > 0:
                    ...
                    if result is not None:
                        mac_addr, x, y, z = result
                        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                        writer.writerow([timestamp, mac_addr, x, y,     z])
                        csvfile.flush()  # Write to disk immediately — safe on crash
                        row_count += 1
                        print(f"  → Saved row {row_count}: t={timestamp} mac_addr={mac_addr} x={x:.3f}  y={y:.3f}  z={z:.3f}")
