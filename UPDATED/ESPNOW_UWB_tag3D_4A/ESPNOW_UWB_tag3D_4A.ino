...
void ESPNOW_Init() {
  ...
  xTaskCreatePinnedToCore(
    ESPNOW_Task,        // Task function
    "ESPNOW_Task",      // Name (for debugging)
...
// For main loop, avoid printing out before all anchor devices are added
bool trilateration_calc_flag = false;
...
void loop()
{
  ...
  // [DEPRECATED]
  // Wait until all devices added once, then begin looping this part of the code
  // if (trilateration_calc_flag) {
  //   data.x = current_tag_position[0];
  //   data.y = current_tag_position[1];
  //   data.z = current_tag_position[2];

  //   ESPNOW_Loop();
  //   trilateration_calc_flag = false;
  // }
}
...
void newRange()
{
  ...
  if ( (detected == N_ANCHORS)) { //four recent measurements
    // Complete handshake here
    trilateration_calc_flag = true;
    ...
    if (dataMutex != NULL && xSemaphoreTake(dataMutex, pdMS_TO_TICKS(10)) == pdTRUE) {
      data.x = current_tag_position[0];
      data.y = current_tag_position[1];
      data.z = current_tag_position[2];
      xSemaphoreGive(dataMutex);
