...
DEBUG_MODE = True          # True to see print statements, False to hide them
        ...
        while True:
            ...
            if len(parts) >= (NUM_ANCHORS * 4):
                ...
                if not any(np.isnan(dists_for_solver)):
                    if FILTER_TYPE == "ekf":
                        # EKF will receive IMU arrays if toggled ON and data is valid, otherwise None
                        # ekf.predict(imu_accel=imu_accel, imu_gyro=imu_gyro) # TODO: ignore for now
                        ekf.predict()
